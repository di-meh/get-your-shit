...
export class OrderController {
  ...
  async getAllByUserId(@Request() req) {
    const orders = await lastValueFrom(
      this.client.send('order-service:getAllByUserId', req.user.sub).pipe(
        catchError((error) => {
  ...
  async getOrderByRestaurantId(@Param('id', ParseUUIDPipe) id: string) {
    const order = await lastValueFrom(
      this.client.send('order-service:getOrderByRestaurantId', id).pipe(
        catchError((error) => {
          throw new InternalServerErrorException(error.message);
        }),
      ),
    ...
    for (let i = 0; i < order.length; i++) {
      const userFromOrder = await this.userService.findOneById(
        order[i].buyerId,
      );
  ...
  async getOrderItem(@Param('id', ParseUUIDPipe) id: string) {
    const orderItem = await lastValueFrom(
      this.client.send('order-service:getOrderItem', id).pipe(
        catchError((error) => {
          throw new InternalServerErrorException(error.message);
        }),
      ),
  ...
  acceptOrder(
    ...
    return this.client.send('order-service:accept', {
      orderCode,
      id,
      deliveryId: req.user.sub,
    });
  }
  ...
  acceptOrderByRestaurant(
    ...
    return this.client.send('order-service:acceptByRestaurant', {
      orderCode,
      id,
      deliveryId: req.user.sub,
    });
  ...
  acceptOrderByUser(
    ...
    return this.client.send('order-service:acceptByUser', {
      orderCode,
      id,
      deliveryId: req.user.sub,
    });
