...
<template>
    ...
    <div class="card w-full lg:grid lg:grid-cols-2 gap-4">
        <div class="card w-full bg-secondary text-secondary-content" v-for="restaurant in restaurants">
            <div class="card-body items-center text-center">
                <h2 class="card-title">{{ restaurant.name }}</h2>
                <p>{{ restaurant.address }}, {{ restaurant.city }}</p>
                <div class="card-actions justify-end">
                    <button class="btn btn-primary">Modifier</button>
                    <button class="btn bg-red-700 border-none" @click="deleteRestaurant(restaurant.id)">Supprimer</button>
                </div>
            </div>
        </div>
    </div>
