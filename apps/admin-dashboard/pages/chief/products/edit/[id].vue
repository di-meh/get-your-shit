...
<template>
  <FormKit v-if="productsRef" type="form" form-class="flex flex-col w-full items-center lg:items-start gap-6"
    :actions="false" @submit="updateProduct($event)">
    <FormKit type="text" name="name" label="Nom du produit" placeholder="Nom du produit" validation="required|name"
    ...
    <FormKit type="number" name="price" label="Prix du produit" placeholder="Prix du produit" validation="required|price"
      input-class="input input-bordered input-accent mt-2 w-full rounded-xl" outer-class="w-full"
      label-class="label label-text" message-class="label-text-alt mt-2 text-error" :value="productsRef.price" />
    <FormKit type="textarea" name="description" label="Description du produit" placeholder="Description du produit"
    ...
    <FormKit v-if="categoriesRef" type="select" label="Catégorie du produit" name="categoryId" outer-class="w-full"
      input-class="input input-bordered input-accent mt-2 w-full rounded-xl" label-class="label label-text"
      message-class="label-text-alt mt-2 text-error" validation="required|categoryId" v-model="productsRef.categoryId">
      <option v-for="cat in categoriesRef" :value="cat.id">{{ cat.name }}</option>
    </FormKit>
    <FormKit v-if="restaurantRef" type="select" label="Restaurant du produit" name="restaurantId" outer-class="w-full"
      input-class="input input-bordered input-accent mt-2 w-full rounded-xl" label-class="label label-text"
      message-class="label-text-alt mt-2 text-error" validation="required|restaurantId"
      v-model="productsRef.restaurantId">
      <option v-for="rest in restaurantRef" :value="rest.id">{{ rest.name }}</option>
    </FormKit>
